...
function setLogLevel(log_level) {
    if (isNotEmpty(log_level) && isKeyInObj(log_level.toUpperCase(), this.LOG_LEVEL)) {
...
function result(callback) {
    if (this.logLevel == LOG_LEVEL.RESULT || this.logLevel == LOG_LEVEL.INFO || this.logLevel == LOG_LEVEL.DEBUG) {
        if (isNotNull(callback)) {
            callback()
        }
    }
...
function warn(callback) {
    if (this.logLevel == LOG_LEVEL.RESULT || this.logLevel == LOG_LEVEL.WARN || this.logLevel == LOG_LEVEL.INFO || this.logLevel == LOG_LEVEL.DEBUG) {
        if (isNotNull(callback)) {
            callback()
        }
    }
...
function error(callback) {
    if (this.logLevel == LOG_LEVEL.ERROR || this.logLevel == LOG_LEVEL.WARN || this.logLevel == LOG_LEVEL.RESULT || this.logLevel == LOG_LEVEL.INFO || this.logLevel == LOG_LEVEL.DEBUG) {
        if (isNotNull(callback)) {
            callback()
        }
    }
