...
class Utils {
    ...
    __delNullLeafNode(list, key) {
        list.forEach(item => {
            item[key] === '' || item[key] === undefined || item[key] === null || item[key].length < 1 ? delete item[key] : __delNullLeafNode(item[key], key)
        })
        return list
    }
    ...
    __diffTime(endDate, type) {
        ...
        switch (type) {
            ...
            case 'minutes':
                ...
                break
            case 'seconds':
                res = days + '天' + hours + '小时' + minutes + '分' + seconds + '秒'
                break
        }
        return res
    }
    ...
    __findNode(tree, func, path, dataStructure) {
        ...
        for (const data of tree) {
            ...
            if (data.children) {
                const findChildren = this.findNode(data.children, func, path, dataStructure)
